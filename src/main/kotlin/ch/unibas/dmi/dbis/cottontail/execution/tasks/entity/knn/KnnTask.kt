...
internal object KnnTask {
    ...
    fun entityScanTaskForPredicate(entity: Entity, knnClause: KnnPredicate<*>, whereClause: BooleanPredicate?) : ExecutionTask {
        val operations = knnClause.query.first().shape()[0] * entity.statistics.rows * (knnClause.operations + (whereClause?.operations ?: 0))
        ...
        return when {
            parallelism > 1 -> ParallelEntityScanKnnTask(entity, knnClause as KnnPredicate<INDArray>, whereClause, parallelism)
            parallelism <= 1 -> LinearEntityScanKnnTask(entity, knnClause as KnnPredicate<INDArray>, whereClause)
